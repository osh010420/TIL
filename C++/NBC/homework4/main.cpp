...
public:
    ...
    void searchRecipeName(const std::string& name) const {
        ...
        if (!found) {
            std::cout << ">> 레시피 '" << name << "'이(가) 존재하지 않습니다." << std::endl;
        }
	}   
};
...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            myWorkshop.searchRecipeName(searchName);
		}
